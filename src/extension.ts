...
const scroll = (direction: ScrollDirection, distance: ScrollDistance) => {
	...
	const { visibleRanges } = editor,
		...
		// conditional
		hasNumberDistance = typeof distance === "number",
		hasFolds =
			firstSegmentDistanceStart < 0 ||
			firstSegmentDistanceEnd < 0 ||
			(firstSegmentDistanceEnd <= scrollOff && ranges.end !== visibleRanges[0].end.line),
		hasScrollOffContact =
			(hasNumberDistance && direction === "down" && scrollOff >= firstSegmentDistanceStart) ||
			(hasNumberDistance && direction === "up" && scrollOff + 1 >= lastSegmentDistanceEnd);

	let distanceValue =
		distance === "halfPage"
			? Math.floor(rangeValue / 2)
			: distance === "page"
			? rangeValue
			: distance;
	...
	if (hasNumberDistance && direction === "down" && rangeValue <= scrollOff + distance) {
		console.log("%cextension.ts line:40 beyondlast", "color: #007acc;");
	...
	// Scroll when lines to top boundary remain
		...
		visibleRanges[0].start.line > 0 &&
		visibleRanges[0].start.line <= scrollOff &&
		direction === "up"
	) {
	...
	// Scroll when visibleRange is mutated through folds
		...
		distanceValue =
			(direction === "down" && hasFoldBelow) || (direction === "up" && hasFoldAbove)
				? distance + scrollOff
				: distance;
	...
	SCROLLERS.forEach((scroller: Scroller) => {
		...
		vscode.workspace.onDidChangeConfiguration(async (event) => {
			...
			if (event.affectsConfiguration("editor.cursorSurroundingLines")) {
				scrollOff = vscode.workspace
					.getConfiguration("editor")
					.get("cursorSurroundingLines") as number;
			}
		});
